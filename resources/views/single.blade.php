...
<div class="px-2 lg:px-0">
    ...
    <!-- Breadcrumbs -->
    <nav class="text-sm breadcrumbs text-gray-500 px-2 mt-2 mb-2">
        <ul>
        <li><a href="{{ route('home', [ 'param1' => $locale_slug, 'param2' => $platform_slug ]) }}" class="text-primary">Home</a></li>
    ...
    <section class="flex flex-col md:flex-row md:items-center md:justify-between space-y-6 md:space-y-0 border-b pb-6 text-center md:text-left">
        ...
        />     
        <div class="space-y-1">
            <h1 class="text-xl font-semibold">{{ $software->name }}</h1>
            ...
            <p class="italic text-sm">{{ $software->softwareTranslations->first()?->tagline }}</p>
        </div>
        </div>
    ...
    <section class="w-full max-w-8xl overflow-hidden relative group mb-4 mt-6">
        <div id="carousel-app" class="carousel flex space-x-4 snap-x overflow-x-scroll scrollbar-hide scroll-smooth">
            <article class="card bg-secondary/10 rounded-2xl min-w-[160px] p-3 flex-shrink-0">
            ...
            <article class="card bg-secondary/10 rounded-2xl min-w-[160px] p-3 flex-shrink-0">
                ...
                <div class="flex flex-wrap gap-1 text-sm">
                    @forelse($software->requirements as $requirement)
                        <span class="badge badge-outline text-xs">{{ $requirement->name }}</span>
                    @empty
                        <span class="text-xs italic text-gray-500">No requirements listed</span>
                    @endforelse
                </div>
            </article>
        </div>
        ...
        <button aria-label="Previous Slide" id="left-btn"
            class="absolute left-0 top-1/4 bg-accent rounded-full border-none shadow p-2 opacity-0 group-hover:opacity-100 hidden btn btn-circle btn-sm
        ...
        <button aria-label="Next Slide" id="right-btn"
            class="absolute right-0 border-none top-1/4 bg-accent rounded-full shadow p-2 opacity-0 group-hover:opacity-100 hidden btn btn-circle btn-sm
            hover:bg-accent transition-all duration-300"
            onclick="scrollCarousel_app(1)">
            ❯
        </button>
    ...
    <!-- Description -->
    <section class="space-y-4 text-base leading-relaxed text-justify pb-4">
    ...
    @if(!empty($software->screenshots) && is_array($software->screenshots) && count($software->screenshots))
        <!-- Screenshots -->
        <section class="overflow-x-auto space-x-4 flex pb-4" aria-label="Software Screenshots">
    ...
    <!-- Related Apps -->
    <section id="new-releases" class="w-full max-w-8xl px-2 overflow-hidden relative group mb-4">
        <h2 class="text-xl font-bold mb-5">{{ $trns->related ?? 'Related Apps' }}</h2>
...
<dialog id="license_modal" class="modal">
  <div class="modal-box">
    ...
    <h2 class="text-lg font-bold">{{ $software->license->licenseTranslations->first()?->name }}</h2>
    <p class="py-4">{{ $software->license->licenseTranslations->first()?->description }}</p>
  </div>
</dialog>
